...
%   for its use
%
% File name: SetUpL22-14vFlashAngles_saveRFallFrames.m
...
Trans.frequency = 22.0;   % The center frequency for the A/D 4xFc sampling.
Trans = computeTrans(Trans);  % L11-5v transducer is 'known' transducer so we can use computeTrans.
% Trans.maxHighVoltage = 50;  % set maximum high voltage limit for pulser supply.

% Specify PData structure array.
PData(1).PDelta = [Trans.spacing, 0, 0.5]; % Pixel spacing in wavelengths
PData(1).Size(1) = ceil((P.endDepth-P.startDepth)/PData(1).PDelta(3)); % Depth size
PData(1).Size(2) = ceil((Trans.numelements*Trans.spacing)/PData(1).PDelta(1)); % Width size
PData(1).Size(3) = 1;                     % Single image page
PData(1).Origin = [-Trans.spacing*(Trans.numelements-1)/2, 0, P.startDepth]; % Origin of the image
...
Resource.DisplayWindow(1).Title = 'L22-14v Flash Angles';
Resource.DisplayWindow(1).pdelta = 0.2;   % Display resolution
ScrnSize = get(0,'ScreenSize');
DwWidth = ceil(PData(1).Size(2)*PData(1).PDelta(1)/Resource.DisplayWindow(1).pdelta);
DwHeight = ceil(PData(1).Size(1)*PData(1).PDelta(3)/Resource.DisplayWindow(1).pdelta);
Resource.DisplayWindow(1).Position = [250, (ScrnSize(4)-(DwHeight+150))/2, DwWidth, DwHeight];
Resource.DisplayWindow(1).ReferencePt = [PData(1).Origin(1), 0, PData(1).Origin(3)]; % Reference point for display
